...
def collate_fn(batch, padding_value=1):
    ...
    return {
        'src': torch.tensor(padded_sources, dtype=torch.long),
        'trg': torch.tensor(padded_targets, dtype=torch.long),
        'src_len': torch.tensor(source_lengths, dtype=torch.long),
        'trg_len': torch.tensor(target_lengths, dtype=torch.long)
    }
